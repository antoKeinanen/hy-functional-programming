...
capitalize s = unwords $ map (\s -> (toUpper $ head s) : tail s) $ words s
...
joinToLength n strings = [a ++ b | a <- strings, b <- strings, (length $ a ++ b) == n]
...
sumRights values = sum $ map (fromRight 0) values
...
interpreter commands = interpreter' (head commands) (tail commands) 0 0

interpreter' :: String -> [String] -> Integer -> Integer -> [String]
interpreter' command [] x y = case command of
  "printX" -> show x : []
  "printY" -> show y : []
  otherwise -> []
interpreter' command commands x y = case command of
  "up" -> interpreter' (head commands) (tail commands) x (y + 1)
  "down" -> interpreter' (head commands) (tail commands) x (y - 1)
  "left" -> interpreter' (head commands) (tail commands) (x - 1) y
  "right" -> interpreter' (head commands) (tail commands) (x + 1) y
  "printX" -> show x : interpreter' (head commands) (tail commands) x y
  "printY" -> show y : interpreter' (head commands) (tail commands) x y
