...
import Data.Ord
import qualified Data.Ord as Array
...
distinct a = (length $ fromList a) == length a
...
rangeOf a = (-) (last sorted) (head sorted)
  where
    sorted = sort a
...
longest :: (Ord a) => [[a]] -> [a]
longest xs = foldl longer [] xs
  where
    longer a b
      | length a == length b = if head a <= head b then a else b
      | otherwise = if length a > length b then a else b
...
incrementKey k = map (incrementKey' k)
  where
    incrementKey' :: (Num v, Eq k) => k -> (k, v) -> (k, v)
    incrementKey' a b = if (==) a $ fst b then (fst b, (+) 1 $ snd b) else b
...
freqs xs = foldr insertOrIncrement Map.empty xs
  where
    insertOrIncrement :: (Ord a) => a -> Map.Map a Int -> Map.Map a Int
    insertOrIncrement value xs =
      if Map.member value xs
        then Map.insertWith (+) value 1 xs
        else Map.insert value 1 xs
...
transfer from to amount bank =
  if amount < 0
    then bank
    else case (Map.lookup from bank, Map.lookup to bank) of
      (Nothing, _) -> bank
      (_, Nothing) -> bank
      (Just fromBalance, _) ->
        if amount > fromBalance
          then bank
          else Map.insertWith (+) to amount $ Map.insert from (fromBalance - amount) bank
...
maxIndex arr = fst $ foldr compareIx (head $ Array.assocs arr) $ Array.assocs arr
  where
    compareIx :: (Ix i, Ord a) => (i, a) -> (i, a) -> (i, a)
    compareIx a b = if snd a > snd b then a else b
