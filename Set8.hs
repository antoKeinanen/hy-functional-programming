...
blendColor a b = Color ((aRed + bRed) `div` 2) ((aGreen + bGreen) `div` 2) ((aBlue + bBlue) `div` 2)
  where
    aRed = getRed a
    aGreen = getGreen a
    aBlue = getBlue a
...
combine f (Picture a) (Picture b) = Picture (\coord -> f (a coord) (b coord))
...
instance Transform Flip where
  apply FlipX (Picture f) = Picture (f . g)
    where
      g (Coord x y) = Coord (-x) y
  apply FlipY (Picture f) = Picture (f . g)
    where
      g (Coord x y) = Coord x (-y)
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where
      g (Coord x y) = Color avgRed avgGreen avgBlue
        where
          center = f (Coord x y)
          up = f (Coord x (y - 1))
          down = f (Coord x (y + 1))
          left = f (Coord (x - 1) y)
          right = f (Coord (x + 1) y)
          avgRed = (getRed center + getRed up + getRed down + getRed left + getRed right) `div` 5
          avgGreen = (getGreen center + getGreen up + getGreen down + getGreen left + getGreen right) `div` 5
          avgBlue = (getBlue center + getBlue up + getBlue down + getBlue left + getBlue right) `div` 5
...
instance Transform BlurMany where
  ...
  apply (BlurMany n) picture = apply Blur $ apply (BlurMany (n - 1)) picture
