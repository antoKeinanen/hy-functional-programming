...
allValues condition (Node a l r) = and [condition a, allValues condition l, allValues condition r]
...
cull val (Node a l r) =
  if a == val
    then Empty
    else Node a (cull val l) (cull val r)
...
isOrdered (Node a l r) =
  and
    [ or
        [ isNothing $ valAtRoot l,
          a > (fromJust $ valAtRoot l)
        ],
      or
        [ isNothing $ valAtRoot r,
          a < (fromJust $ valAtRoot r)
        ],
      isOrdered l,
      isOrdered r
    ]
...
set (first : rest) val (Node a l r) =
  if first == StepL
    then Node a (set rest val l) r
    else Node a l (set rest val r)
...
search target (Node a l r)
  | target == a = Just []
  | otherwise = case search target l of
      Just path -> Just (StepL : path)
      Nothing -> case search target r of
