...
sums' n target
  | n == target = sumTo n : []
  | otherwise = sumTo n : sums' (n + 1) target
...
mylast :: (Eq a) => a -> [a] -> a
mylast def [] = def
mylast def (first : rest)
  | rest == [] = first
  | otherwise = mylast def rest
...
indexDefault :: (Eq a) => [a] -> Int -> a -> a
indexDefault (first : rest) i def
  | i < 0 = def
  | rest == [] && i > 0 = def
  | i == 0 = first
  | otherwise = indexDefault rest (i - 1) def
...
sorted (first : rest)
  | rest == [] = True
  | otherwise = (head rest >= first) && sorted rest
  where
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf (first : rest) = first : sumOf' first rest

sumOf' :: Int -> [Int] -> [Int]
sumOf' _ [] = []
sumOf' x (first : rest) = x + first : sumOf' (x + first) rest
...
merge xs [] = xs
merge [] ys = ys
merge (xFirst : xRest) (yFirst : yRest)
  | xFirst <= yFirst = xFirst : merge xRest (yFirst : yRest)
  | xFirst > yFirst = yFirst : merge (xFirst : xRest) yRest
...
mymaximum bigger initial (first : rest) = mymaximum' first bigger (mymaximum bigger initial rest)

mymaximum' :: a -> (a -> a -> Bool) -> a -> a
mymaximum' lhs op rhs =
  if op lhs rhs
    then lhs
    else rhs
...
map2 f as [] = []
map2 f [] bs = []
map2 f (aFirst : aRest) (bFirst : bRest) = f aFirst bFirst : map2 f aRest bRest
...
maybeMap :: (Eq b) => (a -> Maybe b) -> [a] -> [b]
maybeMap f [] = []
maybeMap f (first : rest)
  | res == Nothing = maybeMap f rest
  | otherwise = maybeUnwrap res : maybeMap f rest
  where
    res = f first

maybeUnwrap :: Maybe a -> a
maybeUnwrap (Just x) = x