...
member v (Set []) = False
member v (Set (first : rest)) = (||) (v == first) (member v $ Set rest)

-- add a member to a set
add :: (Ord a) => a -> Set a -> Set a
add v (Set xs) = Set (insert v xs)
  where
    insert x [] = [x]
    insert x (first : rest)
...
data State = Start | Eggs | Flour | Sugar | UnMixed | UnBaked | Error | Finished
  deriving (Eq, Show)
...
step Eggs _ = Error
step Flour AddSugar = UnMixed
step Flour _ = Error
step Sugar AddFlour = UnMixed
step Sugar _ = Error
step UnMixed Mix = UnBaked
step UnMixed _ = Error
step UnBaked Bake = Finished
step UnBaked _ = Error
step Error _ = Error
...
reverseNonEmpty (first :| []) = first :| []
reverseNonEmpty (first :| rest) = last rest :| (reverse $ init rest) ++ [first]
...
instance (Ord a, Eq a) => Semigroup (Set a) where
  (<>) (Set []) (Set r) = Set r
  (<>) (Set (first : rest)) (Set r) =
    if member first (Set r)
      then Set rest <> Set r
      else add first $ Set rest <> Set r
...
passwordAllowed password (ContainsSome (character : rest)) =
  isInfixOf [character] password
    || passwordAllowed password (ContainsSome rest)
...
data Arithmetic = Literal Integer | BinOp String Arithmetic Arithmetic
  deriving (Show)
...
operation op lhs rhs = BinOp op lhs rhs

evaluate :: Arithmetic -> Integer
evaluate (Literal n) = n
evaluate (BinOp op lhs rhs) = operator (evaluate lhs) (evaluate rhs)
  where
    operator =
      case op of
        "+" -> (+)
...
render (BinOp op lhs rhs) = "(" ++ render lhs ++ op ++ render rhs ++ ")"
